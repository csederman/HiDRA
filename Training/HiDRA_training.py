...
def read_files(GDSC_df, Input_directory):
    ...
    with open("geneset.gmt") as f:
        reader = csv.reader(f)
        data = list(
            list(rec) for rec in csv.reader(f, delimiter="\t")
    ...
    for GeneSet in GeneSet_Dic:
        GeneSet_Dic_withoutNA[GeneSet] = (
            GeneExpression_with_Symbol["ES3"][GeneSet_Dic[GeneSet]]
            .dropna()
            .index.values
        )
    ...
    X_origin = [
        pd.read_csv(Input_directory + str(x) + ".csv", index_col=0)
        for x in range(len(GeneSet_Dic_withoutNA))
    ]  # Read all inputs for cell lines
    X_origin = [
        df.loc[GDSC_without_duplicated["Cell line name"]] for df in X_origin
    ]  # Pick up data for cell lines in given pairs
    Drug = pd.read_csv(
        Input_directory + "drug.csv", index_col=0
    )  # Read drug input
    X_origin.append(
        Drug.loc[GDSC_without_duplicated["Drug name"]]
    )  # Pick up data for drugs in given pairs

    return X_origin


# Make new HiDRA model
def Making_Model(GeneSet_Dic: dict):
    ...
    Drug_Input = Input(
        (Drug_feature_length,), dtype="float32", name="Drug_Input"
    )
    ...
    Drug_Dense_New1 = BatchNormalization(name="Drug_Batch_New1")(
        Drug_Dense_New1
    )
    Drug_Dense_New1 = Activation("relu", name="Drug_RELU_New1")(
        Drug_Dense_New1
    )

    Drug_Dense_New2 = Dense(32, name="Drug_Dense_New2")(Drug_Dense_New1)
    Drug_Dense_New2 = BatchNormalization(name="Drug_Batch_New2")(
        Drug_Dense_New2
    )
    Drug_Dense_New2 = Activation("relu", name="Drug_RELU_New2")(
        Drug_Dense_New2
    )
    ...
    for GeneSet in GeneSet_Dic.keys():
        Gene_Input = Input(
            shape=(len(GeneSet_Dic[GeneSet]),),
            dtype="float32",
            name=GeneSet + "_Input",
        )
        Drug_effected_Model_for_Attention = [Gene_Input]
        # Drug also affects to the Gene-level network attention mechanism
        Drug_Dense_Geneset = Dense(
            int(len(GeneSet_Dic[GeneSet]) / 4) + 1,
            dtype="float32",
            name=GeneSet + "_Drug",
        )(Drug_Dense_New2)
        Drug_Dense_Geneset = BatchNormalization(name=GeneSet + "_Drug_Batch")(
            Drug_Dense_Geneset
        )
        Drug_Dense_Geneset = Activation("relu", name=GeneSet + "Drug_RELU")(
            Drug_Dense_Geneset
        )
        Drug_effected_Model_for_Attention.append(
            Drug_Dense_Geneset
        )  # Drug feature to attention layer

        Gene_Concat = concatenate(
            Drug_effected_Model_for_Attention, axis=1, name=GeneSet + "_Concat"
        )
        # Gene-level attention network
        Gene_Attention = Dense(
            len(GeneSet_Dic[GeneSet]),
            activation="tanh",
            name=GeneSet + "_Attention_Dense",
        )(Gene_Concat)
        Gene_Attention = Activation(
            activation="softmax", name=GeneSet + "_Attention_Softmax"
        )(Gene_Attention)
        Attention_Dot = dot(
            [Gene_Input, Gene_Attention], axes=1, name=GeneSet + "_Dot"
        )
        Attention_Dot = BatchNormalization(name=GeneSet + "_BatchNormalized")(
            Attention_Dot
        )
        Attention_Dot = Activation("relu", name=GeneSet + "_RELU")(
            Attention_Dot
        )
    ...
    Drug_Dense_Sample = Dense(
        int(len(GeneSet_Dic) / 16) + 1,
        dtype="float32",
        name="Sample_Drug_Dense",
    )(Drug_Dense_New2)
    Drug_Dense_Sample = BatchNormalization(name=GeneSet + "Sample_Drug_Batch")(
        Drug_Dense_Sample
    )
    Drug_Dense_Sample = Activation("relu", name="Sample_Drug_ReLU")(
        Drug_Dense_Sample
    )  # Drug feature to attention layer
    Drug_effected_Model_for_Attention.append(Drug_Dense_Sample)
    GeneSet_Concat = concatenate(
        GeneSet_Model, axis=1, name="GeneSet_Concatenate"
    )
    Drug_effected_Concat = concatenate(
        Drug_effected_Model_for_Attention,
        axis=1,
        name="Drug_effected_Concatenate",
    )
    # Pathway-level attention
    Sample_Attention = Dense(
        len(GeneSet_Dic.keys()),
        activation="tanh",
        name="Sample_Attention_Dense",
    )(Drug_effected_Concat)
    Sample_Attention = Activation(
        activation="softmax", name="Sample_Attention_Softmax"
    )(Sample_Attention)
    Sample_Multiplied = multiply(
        [GeneSet_Concat, Sample_Attention], name="Sample_Attention_Multiplied"
    )
    Sample_Multiplied = BatchNormalization(
        name="Sample_Attention_BatchNormalized"
    )(Sample_Multiplied)
    Sample_Multiplied = Activation("relu", name="Sample_Attention_Relu")(
        Sample_Multiplied
    )
...
def main():
    ...
    parser.add_argument(
        "-e", type=str, help="The epoch in the training process"
    )
    parser.add_argument(
        "-o", type=str, help="The output path that model file be stored"
    )
    ...
    training_df = training_pair[["Drug name", "Cell line name"]]
    validation_df = validation_pair[["Drug name", "Cell line name"]]
